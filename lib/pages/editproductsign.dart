...
class editProductSign extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return _editProductSignState();
  }
}

class _editProductSignState extends State<editProductSign> {
    ...
    if (_controllerapi.text != '') {
      if (double.tryParse(_controllerapi.text) != null) {
        saveSign savSign = saveSign(
            ...
            Pieces: _controllerapi.text,
            Token: globalData.token);

        dynamic status = await savSign.post();
        ...
        if (status['ResponseStatus'] == 'Ok') {
          function().showNotify(
            context: context,
            buttonText: Settings.notify['sign_product']['Success']['button'],
            message: Settings.notify['sign_product']['Success']['message'],
            title: Settings.notify['sign_product']['Success']['title'],
          );
        ...
        function().showNotify(
          context: context,
          buttonText: Settings.notify['sign_product']['notNumber']['button'],
          message: Settings.notify['sign_product']['notNumber']['message'],
          title: Settings.notify['sign_product']['notNumber']['title'],
        );
      }
    } else {
      function().showNotify(
        context: context,
        buttonText: Settings.notify['sign_product']['Empty']['button'],
        message: Settings.notify['sign_product']['Empty']['message'],
        title: Settings.notify['sign_product']['Empty']['title'],
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        backgroundColor: Settings.theme['color'],
        title: Text(Settings.language['Edit_Product_Sign']['Title']),
        centerTitle: true,
      ),
      body: Container(
        child: Column(
                      ...
                      child: OutlineButton(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(
                              Settings.language['Edit_Product_Sign']
                                  ['Button_Save'],
                              style: TextStyle(
                                  color: Settings.theme["color"], fontSize: 18),
                            ),
                          ],
                        ),
                        color: Settings.theme["font_color"],
